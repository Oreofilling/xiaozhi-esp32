#include <esp_log.h>
 ...
 #define TAG "CameraNe101"
 
 CameraNe101::CameraNe101() {
 ...
 const char* CameraNe101::GetFormat(void)
 {
     return format_2_string(format_);
 }
 
 int CameraNe101::GetWidth(void)
 {
     return width_;
 }
 
 int CameraNe101::GetHeight(void)
 {
     return height_;
 }
 
 int CameraNe101::GetBufferSize(void)
 {
     return buf_size_;
 }
 
 /* only support jpeg format currently*/
 void* CameraNe101::Capture(const char* format) 
 {
     //skip 3 frames to let the sensor output stable and uptodate
     ESP_LOGI(TAG, "capture ne101 image");
     int count = 3;
     while(count--) {
         camera_fb_t *pic = esp_camera_fb_get();
         if(pic) {
             ESP_LOGI(TAG, "capture ne101 image success,count:%d",count);   
     ...
     camera_fb_t *pic = esp_camera_fb_get();
     if(!pic) {
         ESP_LOGE(TAG, "capture ne101 image failed");
         return nullptr;
     }
 ...
 bool CameraNe101::Lock(int timeout_ms) {
     return true;
 }
 
 void CameraNe101::Unlock() {}